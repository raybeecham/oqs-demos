...
def run_docker_container(image_name: str = "openquantumsafe/curl", additional_args: str = '', env_vars: dict = None):
    """
    Run a Docker container from the specified image with optional environment variables.
    """
    env_args = ' '.join([f"-e {k}={v}" for k, v in env_vars.items()]) if env_vars else ''
    try:
...
def query_server(curves: str, url: str = "https://localhost:4433"):
    """
    Query the server using the specified curves.
    """
    try:
...
def run_tls_handshake_performance_test(test_time: int = 200, kem_alg: str = "kyber768", sig_alg: str = "dilithium2"):
    """
    Run a TLS handshake performance test with specified parameters.
    """
    run_docker_container(additional_args="-it", env_vars={"TEST_TIME": str(test_time), "KEM_ALG": kem_alg, "SIG_ALG": sig_alg}, image_name="openquantumsafe/curl perftest.sh")
...
def run_algorithm_performance_test(seconds: int = 2, alg: str = ""):
    """
    Test the performance of specific algorithms.
    """
    command = f"docker run -it openquantumsafe/curl openssl speed -seconds {seconds} {alg}"
    try:
        subprocess.run(command, shell=True, check=True)
    except subprocess.CalledProcessError as e:
